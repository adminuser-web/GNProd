...
**Git:** `main` HEAD = `a71adc2`. `develop` == `main` (all work published; working tree clean). ~88 unit tests pass; `npm run lint`/`test`/`build` all green. Deploy gate: commit to develop, merge→main ONLY on explicit user approval.

**LIVE domain:** `grainood.com` now points to Vercel project **gn-prod** (apex `A @ → 216.198.79.1`, `CNAME www → cname.vercel-dns.com`; nameservers stay Hostinger `*.dns-parking.com`). Email = **Google Workspace** (`MX @ → smtp.google.com`, `TXT @ SPF → v=spf1 include:_spf.google.com ~all`, `TXT google._domainkey` DKIM, `TXT _dmarc → v=DMARC1; p=none`). ⚠️ Do NOT click Hostinger "Reset DNS zone" (re-adds Hostinger A/MX and breaks both). Google MX accepts `adminuser@grainood.com` + `connect@grainood.com` (verified via SMTP RCPT probe).

**Unified Attribute model (Phases 1–5, all SHIPPED + prod data migrated):**
- `src/features/products/attributes.ts` — `ProductAttribute {id,key(kebab),label,mode:'fixed'|'customizable',sortOrder,active,fixedValue?,required?,type?,options?,maxLength?,validationRegex?}`. Helpers: `deriveAttributes` (from legacy specs+customizationGroups; a spec is HIDDEN only when `SPEC_TO_CUSTOMIZATION_MAP[field]` maps to an active customizable group — else it's a fixed attr with a unique key), `getAttributes`/`getFixedAttributes`/`getCustomizableAttributes` (active = `enabled!==false && active!==false`), `applySeriesDefaults` (gap-fill by key, added forced INACTIVE), `attributeProvenance` (series-default/overridden/product), `validateAttributes` (no empty/dup keys, required customizable needs ≥1 available option).
- Readers all use attributes: `lib/pricing.ts` (computePrice iterates customizable attrs), `useProductConfigurator`, `ProductPage` (configurator + spec sheet + merge sets `attributes`), consultant, MyBuilds, Comparison, Collection, OrderDetail. Legacy `specs`/`customizationGroups` RETAINED on docs for rollback.
- ⚠️ **Options are gated by `available` (NOT `active`)** in configurator + pricing — real option data has `available:true` with `active` unset; the old `!opt.active`/`group.active` checks broke add-to-cart AND meant option surcharges never applied. Fixed. **Pricing now charges option priceDeltas** (engraving +1500, etc.) — this was a deliberate approved behavior change.
- Prod migrations RUN via `scripts/migrate-attributes.ts --apply` (271→271 options, 0 loss) + `scripts/fill-series-defaults.ts --apply` (49 inactive attrs added). Series-level `attributes` = the template (13 customizable each). Reports in `docs/`.
- Admin: unified **Attributes tab** (`AdminAttributesTab.tsx`, reusable) = collapsible sectioned rows (All/Options/Specs tabs + search + expand/collapse all), drag+up/down reorder, inline validation, "Apply Series Defaults". Series template edited on `AdminSeriesDetailPage` ("Series Default Attributes"). Old AdminSpecsTab/AdminCustomizationsTab DELETED. Editor blocks Save on attribute errors.
- **Duplicate** a sub-series: `src/features/products/duplicate.ts` `buildDuplicateSubSeries` (copies attributes/pricing/SEO, fresh id/slug/SKU, active:false, **references same image URLs — never re-uploads**). Buttons on series list + editor header.

**Customer ProductPage redesign:** sub-series switcher REMOVED → `CompareDrawer.tsx` (side panel/slide-up, lists other sub-series w/ price+key fixed attrs + "View this bat"; dormant until a series has 2+ subs). Reviews via `useContent('reviews')` + empty state (no fabrication); craft/care + value-prop blocks. **Collection cards use `series.imageUrl` (primary), not heroImage.**

**Order page (admin):** `lib/orderTimeline.ts` (`buildStatusTimeline`/`formatDuration`/`toDate`) → Jira-style status header + vertical timeline with per-bucket durations (live "counting"), from `order.timeline` events (no data-model change). `AdminOrderDetailsPage` shows it; payment-confirm errors now surfaced.

**Maintenance / "Launching Soon" mode (SHIPPED, default OFF):**
- Content area `maintenance` in `SiteContentMap` (`{enabled,headline,subtext,heroImage?,bypassSecret}`); DEFAULT enabled:false so it never blacks out the site on deploy. Admin → Content → **Site Status** tab (in `AdminContentEditorPage`; NOTE the sidebar renders from a hardcoded `areas` array — must include new areas).
- `MaintenanceGate.tsx` (in `App.tsx`, inside providers+Router): when ON, public routes render `ComingSoon.tsx`; `/admin/*` + `/login` always open; bypass = admin OR `?preview=<secret>` (stored in localStorage, stripped from URL; `lib/maintenance.ts`). Waits for content+auth to avoid flash.
- `ComingSoon.tsx` = cricket-themed animated splash: real logo + **GRAINOOD wordmark** (gold gradient) + shimmer headline (left), hero bat photo with pulsing gold glow (right, `mix-blend-mode:screen` drops the photo's dark bg so no rectangle — needs a black/transparent-bg image), gentle float, gold dust; responsive (stacks/centers on mobile); `prefers-reduced-motion` safe. Hero image editable in Site Status (default = Debutant cover URL).

**Favicon fix:** `SiteMeta.tsx` removes+recreates `<link rel=icon>`+`apple-touch-icon` and syncs MIME type to the file ext (Safari was dropping the PNG because the static link declared `image/svg+xml`). Safari caches favicons hard — needs cache clear to show.

**Admin/creds pattern:** one-off `scripts/*.ts` (tsx) auth as `adminuser@grainood.com` / `Cricket@123_` + a **fresh TOTP** (`is_admin()` needs aal2). TOTP expires in ~30s — pre-stage the script and pass `TOTP=<code>` env so it authenticates instantly. `orders` RLS has **no admin-INSERT policy** (insert requires `user_id=auth.uid()`) — flagged, deferred (only matters if an "admin creates order for walk-in" feature is added). Prod images: brand logo `content/brand/logo/1782718236879-GrainoodLogo-2.png`, debutant cover `products/debutant/1782887114344-DebutantCover.png`.

**Open / deferred:** demo order `TEST-FLOW-MR1Q940V` left in prod (`delete from public.orders where id='TEST-FLOW-MR1Q940V';` to remove). User to (a) toggle Site Status ON when ready to launch, (b) verify email send/receive after DNS propagation, (c) finish DKIM "Start authentication" in Google Admin. Backlog from before still open: paginate admin Orders/Customers, Turnstile on public forms, remove unused `@google/genai`.
