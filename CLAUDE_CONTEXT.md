...
## Admin-driven payment-by-email (on `develop`)
On-demand, admin-reviewed order emails (NOT the auto-confirmation email we removed). Provider: **Resend** via a Supabase edge function. Turnstile for enquiries = **deferred follow-up** (not built; `verify-turnstile` + ContactPage wiring still TODO; `VITE_TURNSTILE_SITE_KEY`/`TURNSTILE_SECRET_KEY` documented in `.env.example`).
- `supabase/functions/send-order-email/` — verifies caller JWT + admin (`profiles.role`), loads order via **service role**, reads customer email/name **server-side only**, validates template allow-list (`payment_request`|`payment_received`) + `https` link, sends via Resend (`EMAIL_API_KEY` secret + `EMAIL_FROM`), returns generic ok/fail. **No PII in response or logs.** Client (`orderEmailService.sendOrderEmail`) sends ONLY `{orderId, template, paymentLink?}`. `deno check` passes.
- `supabase/functions/_shared/orderEmailTemplates.ts` — single templates module (HTML-escaped, branded). `_shared/cors.ts` re-added.
- `migration 20260630000001` — **private `payment-proofs` bucket** + admin-only storage RLS; SECURITY DEFINER **`orders_notify_admin` trigger** creates a PII-free admin notification on every new order (replaces the old client-side notif in `orderService.createOrder`).
- `src/lib/storage.ts` — `uploadPrivate()` + `getSignedUrl()` (proofs are private; admin views via short-lived signed URL).
- Admin `AdminOrderDetailsPage` — "Send Payment Email" preview modal (name + summary + required https link) · Payment tab "Upload Proof & Confirm" → uploads to private bucket, confirms payment (`confirmedBy`/`confirmedAt`), advances order to **Processing** (`Payment Confirmed → Processing` is an allowed transition) · "Payment received" one-click email.
- **PII hygiene**: "Open Customer 360" now passes `userId` via **router state** (was `?userId=&orderId=` query string); `AdminCustomersPage` reads `location.state` (query fallback). Sanitized the enquiry error log.
- Security audit findings: **RLS already ON for all 16 tables**, `is_admin()` SECURITY DEFINER + `prevent_role_change` exist. `notifications` insert policy is permissive (`with check (true)`) — acceptable (admin-read only) but could be tightened later. Legacy duplicate tables exist (`support_tickets` vs `tickets`, `series` vs `products`) — RLS on, likely orphaned.
- **ACTION (user):** (1) `supabase functions deploy send-order-email`; (2) `supabase secrets set EMAIL_API_KEY=... EMAIL_FROM=... SITE_URL=... BRAND_NAME=...`; (3) run migration `20260630000001` in SQL editor; (4) Resend domain DNS (SPF/DKIM). Firebase remnants are cosmetic-only (comments + `handleFirestoreError` name).
